...
Esta capability define una Home pública y limitada a la bienvenida y Login, sin exponer navegación ni acciones privadas antes de autenticar, y conserva la experiencia completa y la protección del estudio para participantes autenticados. La bienvenida comunica que el estudio clasifica razones de rework en pull requests generados por agentes de IA.

## ADDED Requirements

### Requirement: Home anónima limitada a bienvenida y Login

La Home SHALL mostrar a las personas anónimas únicamente una bienvenida explicativa y exactamente una acción Login. La bienvenida MUST mostrar este párrafo canónico, con su texto visible exacto: `Labeling is a research study that classifies reasons for rework in pull requests generated by AI agents. Your work is private to you. Log in to begin reviewing PR cards and track your progress.`. El marcado MUST conservar exactamente `<span class="text-nowrap">track your progress.</span>` para mantener juntas esas palabras en pantallas estrechas.

#### Scenario: Marcado de bienvenida para persona anónima

- **WHEN** una persona sin una sesión válida solicita Home
- **THEN** Home muestra el párrafo canónico completo, la bienvenida y exactamente una acción Login
