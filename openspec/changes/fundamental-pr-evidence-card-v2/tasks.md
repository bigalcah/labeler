...
- [x] 6.6 Configurar `labeling-study-prepare` para montar un manifiesto de cuentas desde `STUDY_ACCOUNT_MANIFEST_HOST_PATH` en una ruta interna fija, exponer solo `STUDY_ACCOUNT_MANIFEST_FILE` al prepare y excluir ambos del servidor web.
- [x] 6.7 Actualizar la plantilla y documentación de despliegue para generar el manifiesto fuera de Docker, protegerlo con `0400`, configurar su ruta absoluta y no almacenar contraseñas de participantes en `.env`.
...
- [x] 7.4 Añadir pruebas de configuración que rechacen un manifiesto ausente o expuesto al servidor web y validen el montaje read-only exclusivo del prepare.
