...
test("anonymous Home emits only the welcome and one Login action despite client-supplied identity data", async () => {
    ...
    try {
        ...
        assert.match(html, /<p class=(?:"lead"|lead)>Labeling is a research study that classifies reasons for rework in pull requests generated by AI agents\. Your work is private to you\. Log in to begin reviewing PR cards and <span class=(?:"text-nowrap"|text-nowrap)>track your progress\.<\/span><\/p>/);
...
test("public login is accessible, non-enumerating, and rejects missing or invalid CSRF before authentication", async () => {
    ...
    try {
        ...
        assert.doesNotMatch(html, /<header\b/);
        assert.doesNotMatch(html, /<nav\b[^>]*\bclass=(?:"[^"]*\bnavbar\b[^"]*"|[^\s>]*\bnavbar\b[^\s>]*)/);
        assert.doesNotMatch(html, /<a\b[^>]*\bhref=(?:["'])?https:\/\/seart\.si\.usi\.ch(?:["'])?[^>]*\bnavbar-brand\b/);
        assert.doesNotMatch(html, /<(?:a|form)\b[^>]*\b(?:href|action)=(?:["'])?\/(?:queue|progress|logout)(?:["'])?/);
        assert.match(html, /<footer\b[^>]*>\s*<div class=(?:"container text-center"|container\s+text-center)>[\s\S]*?<\/footer>/);
        assert.match(html, /<form\b(?=[^>]*\bid=(?:"login-form"|login-form))(?=[^>]*\baction=(?:"\/login"|\/login))(?=[^>]*\bmethod=(?:"post"|post))(?=[^>]*\bautocomplete=(?:"on"|on))[^>]*>/);
        assert.match(html, /<label for=username>Username<\/label>/);
        assert.match(html, /<input id=username\b[^>]*name=username[^>]*type=text[^>]*autocomplete=username[^>]*required/);
        assert.match(html, /<label for=password>Password<\/label>/);
        assert.match(html, /<input id=password\b[^>]*name=password[^>]*type=password[^>]*autocomplete=current-password[^>]*required/);
        assert.match(html, /<input type=hidden name=csrf_token value=[^ >]+>/);
